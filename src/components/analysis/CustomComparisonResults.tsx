...
import { TrendingUp, Package, AlertCircle, AlertTriangle, CheckCircle2 } from 'lucide-react';
...
export function CustomComparisonResults({ data, timestamp, brandA, brandB }: Props) {
  const getCompetitivenessIcon = (score: number | null | undefined) => {
    if (score === null || score === undefined) return <AlertCircle className="h-5 w-5 text-muted-foreground flex-shrink-0" />;
