...
export function CustomComparisonResults({ data, timestamp, brandA, brandB }: Props) {
  ...
  const getCompetitivenessColor = (score: number | null | undefined) => {
    if (score === null || score === undefined) return 'text-gray-700 bg-gray-50';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-50 rounded-lg p-4 text-sm text-slate-600">
        <div className="flex items-center gap-4">
          ...
          <span>
            <strong>Comparison:</strong> {brandA} vs {brandB}
          </span>
          <span>
            <strong>Currency:</strong> {data.currency || 'GBP'}
          </span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-6">
        ...
        {data?.overall_competitive_sentiments && data.overall_competitive_sentiments.length > 0 ? (
          <div className="space-y-4">
            {data.overall_competitive_sentiments.map((sentiment: any, idx: number) => (
              <div
                key={idx}
                className={'p-4 rounded-lg border-2 ' + getScoreColor(sentiment?.score)}
              >
        ...
        {data.brand_a_products_analysis?.map((product: any, idx: number) => (
          ...
          >
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2 text-sm">
                ...
                <span className={'px-2 py-1 rounded font-medium ' +
                  getCompetitivenessColor(product?.product_breakdown?.competitiveness_score)}>
            ...
            {product?.price_suggestions && product.price_suggestions.length > 0 && (
              <div>
                <h4 className="font-semibold text-slate-900 mb-2">Price Recommendations</h4>
                <div className="space-y-2">
                  {product.price_suggestions.map((suggestion: any, pidx: number) => (
                    <div key={pidx} className="bg-green-50 border border-green-200 rounded p-3 text-sm">
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="text-right">
                          ...
                          <div className={'text-xs mt-1 px-2 py-0.5 rounded ' +
                            getCompetitivenessColor(comp?.competitiveness_score)}>
